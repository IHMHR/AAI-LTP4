...
public class Vendas
{
    ...
    public static ResultSet listaVendas() throws ErrorHandle, IOException
    {
        ...
        try
        {
            retorno = Banco.Selecionar("v.codVenda, ven.cod_vendedor, ven.nome_vendedor, c.codCliente, c.nome, v.data_venda", TABLE_NAME + " v INNER JOIN vendedores ven ON v.cod_vendedor = ven.cod_vendedor INNER JOIN clientes c ON c.codCliente = v.codCliente");
        }
        catch (ErrorHandle | ClassNotFoundException e)
        {
            FileWriter arq = new FileWriter("erroLog.txt", true);
